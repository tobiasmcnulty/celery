            ...
            if option.takes_value():
                ...
                if len(rargs) < nargs:
                    if nargs == 1:
                        ...
                        self.error(
                            "{0} requires {1} arguments".format(opt, nargs))
                elif nargs == 1:
                    value = rargs.pop(0)
                else:
...
class detached_celeryd(object):
    ...
    def parse_options(self, prog_name, argv):
        ...
        if options.logfile:
            parser.leftovers.append("--logfile={0.logfile}".format(options))
        if options.pidfile:
            parser.leftovers.append("--pidfile={0.pidfile}".format(options))
